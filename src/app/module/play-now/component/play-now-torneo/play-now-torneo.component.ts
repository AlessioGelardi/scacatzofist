...
export class PlayNowTorneoComponent {
  ...
  create() {
    if (this.createTorneoForm.valid) {
      ...
      const typeTournament = Number(this.createTorneoForm.value.type);
      if(regCostCoins!==0 || regCostCredits!==0) {

        if((typeTournament === TipologieTorneo.ELIMINATORIE && regCostCoins!>=500 && regCostCredits!>=15) || (typeTournament === TipologieTorneo.CLASSIFICATO && regCostCoins!>=2000 && regCostCredits!>=50)) { //costo minimo
          if(orgPart) {
          ...
          if(typeTournament === TipologieTorneo.ELIMINATORIE) {
            this.messageService.alert('Attenzione!','Il costo minimo di partenza per un torneo ELIMINATORIE è 500 coin 15 crediti','info');
          } else if(typeTournament === TipologieTorneo.CLASSIFICATO) {
            this.messageService.alert('Attenzione!','Il costo minimo di partenza per un torneo CLASSIFICATO è 2000 coin 50 crediti','info');
          }
        }
      } else {
